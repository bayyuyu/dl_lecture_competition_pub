...
class VQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        question = [self.question2idx[process_text(question["question"])] for question in self.df["questions"][idx]]
...
class VQAModel(nn.Module):
    ...
    def forward(self, image, input_ids, attention_mask):
        # Process the question with BERT
        outputs = self.bert_model(input_ids=input_ids, attention_mask=attention_mask)
        question_embeds = outputs.last_hidden_state[:, 0, :]  # CLSトークンの出力を使用

        # Combine the image and question embeddings
        combined = torch.cat((image, question_embeds), dim=1)
